...
class filter_window:
    """
        A class for the filtering window.

        Attributes:
        -----
            gui: tkinter GUI
            numerical_attr (dict): a dictionary of the attributes.
            yes_no_attr (dict): a dictionary of whether the attributes are filtered or not.
            score_range (tuple): a tuple of the range of scores.
            number_papers (int): number of proposals includes.
            default_dict (dict): a dictionary of the default attributes.

        Methods:
        -----
            create_wigets_yesno(order):
                Choosing whether an attribute is filtered or not.
            create_wigets(label,order):
                Choosing the range of scores to included.
            return_pairs():
                Transport the pairs filtered to the main GUI.
            show():
                Run the filtering window.
    """
    def __init__(self, gui, numerical_attr, yes_no_attr, score_range, num_papers, default_dict=None) -> None:
        self.gui = gui
        self.root2 = Tk()
    ...
    def create_wigets_yesno(self, order):
        """
            ...
            Parameters:
            -----
                order (int): order of yes no variable.
        """
    ...
    def create_wigets(self, label, order):
        """
            ...
            Parameters:
                ...
                order (int): order of the variable to display.
        """
        # padding for widgets using the grid layout
        paddings = {'padx': 2, 'pady': 5}
        # label
